...
class PidClaimError(Exception):
    def __init__(self, message, pid=None, fname=None):
...
class pidclaim():
    ...
    def __init__(self, pidfile=None, **params):
        ...
        if self.active_pidfile is None:
            raise PidClaimError("Existing %r process %s running" % (pidfile, curpid), curpid, pidfile)
        ...
        if pidfile_exists:
            try:
                ...
                raise PidClaimError("Unable to remove existing %r for zombie process" % self.active_pidfile, curpid, pidfile)
        self.temp_pidfile = self.active_pidfile + '.' + str(pid) + '.tmp'

        if os.path.exists(self.temp_pidfile):
            raise PidClaimError("Temp pidfile %r already exists" % self.temp_pidfile, curpid, pidfile)
            ...
            try:
                ...
                if attempts > 3:
                    try:
                        ...
                        pass
                    raise PidClaimError("Symlink of %r failed after %s attempts", (self.temp_pidfile, attempts), curpid, pidfile)
        ...
        try:
            ...
            try:
                ...
                pass
            raise PidClaimError("Symlink rename of %r failed after %s attempts" % (self.temp_pidfile, attempts), curpid, pidfile)
